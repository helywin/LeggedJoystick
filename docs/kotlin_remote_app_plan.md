...
`Standard-10inch_A2` 上还存在系统服务 `com.siyi.udpservice`，监听本机 `127.0.0.1:19856`，并通过 `/dev/ttyHS3` 做串口桥接。新 App 可以绑定该服务并请求打开桥，因此当前默认 UDP 入口改成本机地址。但这个服务不能证明 UDP 与串口是独立通道；它可能和其他 App 直接读 `/dev/ttyHS3` 竞争同一条数据流。后续只有在“本 App 走本机 UDP、其他 App 直接读串口”同时稳定收到有效 `0x42` 通道帧，并验证启动、停止、重连、改频互不影响后，才能认为两路可并行。

当前已验证 App 能持续向本机 UDP 桥发送 `HZ_50` 通道订阅，并能收到有效 `CMD_ID = 0x42` 通道帧。未完成 UDP/串口独立性和 `freq = 0` 影响范围验证前，不要发送 `freq = 0` 关闭通道输出。
