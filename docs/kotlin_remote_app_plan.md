...
未纳入方案的素材：充电桩、厂商配网、厂商 SDK 状态、原遥控器硬件按键映射、AR8030/Skydroid 图标和逻辑、`icon_over_mouse*.png`、`icon_small_spinning_top*.png`。`过挡鼠板` 在 `legged_driver` 官方 SDK 中没有对应接口；`扭一扭` 实测与 SDK `COMMAND_CODE_GAIT` 语义不一致，新 App 不实现主屏入口。
...
当前本机探测结论是：UDP 同端口复用不能让两个进程都收到同一份单播数据。因此多 App 共用摇杆数据时，不应设计为“多个 App 绑定同一个本地 UDP 端口”。不同客户端 UDP 端口也已实测不适合作为稳定多订阅方案：两个端口都能收到部分帧，但 `com.siyi.udpservice` 更接近最近客户端地址模型，不会稳定广播给多个端口。因此多 App 共用应使用一个采集者独占 UDP 或串口，再通过绑定服务或受限广播分发。

`Standard-10inch_A2` 上还存在系统服务 `com.siyi.udpservice`，监听本机 `127.0.0.1:19856`，并通过 `/dev/ttyHS3` 做串口桥接。新 App 可以绑定该服务并请求打开桥，因此当前默认 UDP 入口改成本机地址。实测已经确认：本 App 走本机 UDP 桥、另一个进程直接读 `/dev/ttyHS3` 时，两边会竞争同一条串口字节流，串口完整帧数量从基线 37 帧降到 6 帧，App 端也出现输入超时和恢复抖动。因此这两路不能作为多个 App 的独立输入通道。

当前已验证 App 能持续向本机 UDP 桥发送 `HZ_50` 通道订阅，并能收到有效 `CMD_ID = 0x42` 通道帧。`freq = 0` 串口直测未发现会停止 `/dev/ttyHS3` 通道输出，但第一版仍不主动发送 `freq = 0` 关闭通道输出。多个 App 共用摇杆数据时，应改为单采集者分发，而不是让一个 App 用本机 UDP 桥、另一个 App 直接读串口。

Android 广播不适合作为当前原始摇杆数据主分发链路：它不是 UDP 端口模型，别的 App 无法只换端口就解析；跨 App 后台接收受系统限制，高频二进制帧还会受 Binder 事务大小影响。当前 App 应独占 UniRC 原 UDP 输入，并把收到的每个原始 UDP datagram 在解析前原样转发到本机可配置 UDP 端口，默认 `127.0.0.1:19857`，其他开发者监听该端口后自行复用原始协议解析。
...
| 区域 | 内容 |
| --- | --- |
| 背景层 | 默认显示头部视频流 `rtsp://192.168.234.1:8554/front` 或占位画面，使用铺满策略覆盖主屏可用区域；点击左上小视频窗后与尾部视频流互换 |
| 左上小视频窗 | 默认显示尾部视频流 `rtsp://192.168.234.1:8554/back`，控件固定为 16:9；点击后只在 App 本地互换主背景和小窗视频源，不跟随头尾方向状态自动切换 |
| 顶部左侧 | 返回/退出控制、连接状态 |
| 顶部中间 | 运动模式选择，默认普通模式，支持普通/原地/楼梯 |
| 顶部右侧 | 网络状态、补光灯、拍照/录像、设置入口 |
| 左侧中部 | 速度等级按钮，显示低速/中速/高速；点击后显示三段垂直速度选择器 |
| 左侧下部 | 当前线速度读数 |
| 底部居中 | 第一个按钮负责展开/收缩动作组；展开后显示站立、匍匐、卧倒、爬高墙、过窄墙、锁定 |
