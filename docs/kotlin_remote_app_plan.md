...
当前已验证 App 能持续向本机 UDP 桥发送 `HZ_50` 通道订阅，并能收到有效 `CMD_ID = 0x42` 通道帧。未完成 `freq = 0` 影响范围验证前，不要发送 `freq = 0` 关闭通道输出。多个 App 共用摇杆数据时，应改为单采集者分发，而不是让一个 App 用本机 UDP 桥、另一个 App 直接读串口。
