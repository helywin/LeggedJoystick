...
- 不同客户端 UDP 端口已经实测：两个端口都能单独订阅，并发重复订阅时也能收到部分帧，但 `com.siyi.udpservice` 更接近最近客户端地址模型，不会稳定广播给多个端口。因此多 App 共用必须使用一个采集者独占 UDP 或串口，再通过本机 UDP 原始报文转发提供给其他监听端口。
...
- 顶部在连接按钮左侧保留自动/手动 AppMode 切换入口，并直接显示“自动模式”或“手动模式”文字，保留连接、电量、拍照和设置入口；左上角不再显示 App 标题和输入状态文字。模式按钮只切换机器人自动/手动模式，不承担接管或释放控制权。
