...
- RTSP 视频运行时归属于进程级 `RtspVideoRuntime`，复用单个 `LibVLC` 实例，并按主背景、小窗两个槽位稳定持有播放器；Activity 后台时停止并解绑视频输出，恢复前台时复用现有 `AndroidView` 重新绑定 `VLCVideoLayout` 并重新加载当前 RTSP 地址，避免 Surface 复用黑屏和 Compose 重建视图导致 VLC 重复创建 video output。
- RTSP 视频按实时遥控预览处理，VLC 不打开 `-vv` verbose 日志，不强制保留迟到帧；网络或输出抖动时优先丢弃迟到帧，避免解码堆积造成日志风暴、卡顿或 ANR。
